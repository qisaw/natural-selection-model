...
describe('getEnergyConsumption', () => {
  it('should use the e=mv^2 * sense', () => {
    ...
    const playerTwo = createPlayer({ position, speed: 2, sense: 1 });
    ...
    const playerThree = createPlayer({ position, speed: 2, sense: 2 });
    expect(getEnergyConsumption(playerThree)).toEqual(8);
  });
});
