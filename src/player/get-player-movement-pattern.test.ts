...
describe('getPlayerMovementPattern', () => {
  it('should return an object with the keys as ids of the players passed in', () => {
    ...
    expect(Array.from(getPlayerMovementPattern(players, 3).keys()).sort()).toEqual(players.map(({ id }) => id).sort());
  });
  it('should return true the same number of times as the speed of the player', () => {
    ...
    expect((values.get(playerOne.id) || []).sort()).toEqual([false, false, true]);
    expect((values.get(playerTwo.id) || []).sort()).toEqual([false, true, true]);
    expect((values.get(playerThree.id) || []).sort()).toEqual([true, true, true]);
  });
  it('should only return the numOfItems as specified in maxTurns', () => {
    ...
    const values = getPlayerMovementPattern(players, 3);
    expect((values.get(playerOne.id) || []).sort()).toEqual([true, true, true]);
  });
});
